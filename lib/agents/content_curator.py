...
from lib.persona.swizz_persona import SwizzPersona, JordanWardPersona
...
class ContentCurator:
    ...
    FORMAT_SIGNALS = {
        'bridge_builder': {
            'signals': [
                'everyday use', 'anyone can', 'simple', 'accessible',
                'small business', 'non-technical', 'regular people',
                'save time', 'save money', 'practical', 'real world',
                'barber', 'restaurant', 'aunt', 'uncle', 'neighbor',
            ],
        ...
        'real_talk': {
            'signals': [
                'personal', 'story', 'lesson', 'grew up', 'experience',
                'changed', 'perspective', 'journey', 'struggle', 'growth',
                'reinvent', 'rebuild', 'faith', 'grateful', 'humble',
            ],
        ...
        'ask_the_audience': {
            'signals': [
                'question', 'opinion', 'debate', 'controversial', 'poll',
                'what would you', 'agree or disagree', 'hot take',
                'unpopular opinion', 'discussion', 'thoughts',
            ],
        ...
        'problem_solution': {
            'signals': [
                'problem', 'solution', 'fix', 'broken', 'inefficient',
                'waste', 'cost', 'expensive', 'manual', 'automate',
            ],
        ...
        'myth_busting': {
            'signals': [
                'myth', 'wrong', 'misconception', 'actually', 'truth',
                'people think', 'assume', 'overrated', 'underrated',
            ],
        ...
        'quick_tips': {
            'signals': [
                'tips', 'list', 'steps', 'ways', 'rules', 'lessons',
                'hack', 'shortcut', 'framework', 'checklist',
            ],
        ...
        'industry_commentary': {
            'signals': [
                'trend', 'industry', 'market', 'prediction', 'future',
                'shift', 'disruption', 'competition', 'landscape',
                'announcement', 'launch', 'acquisition', 'funding',
            ],
        ...
        'vibe_coder': {
            'signals': [
                'code', 'built', 'shipped', 'commit', 'deploy',
                'cli', 'terminal', 'dev', 'programming', 'self-taught',
                'no degree', 'learned', 'stack', 'framework',
            ],
        ...
        'case_study': {
            'signals': [
                'client', 'results', 'before after', 'transformation',
                'roi', 'outcome', 'delivered', 'project', 'built for',
            ],
        ...
        'day_in_life': {
            'signals': [
                'morning', 'routine', 'behind the scenes', 'day',
                'workflow', 'setup', 'workspace', 'process',
            ],
    ...
    def analyze_angle(
        ...
        if any(w in desc_lower for w in ['business', 'startup', 'revenue', 'client', 'money', 'hustle']):
            themes.append('business')
        if any(w in desc_lower for w in ['learn', 'school', 'education', 'teach', 'student', 'access']):
    ...
    def _find_story_connections(self, description: str) -> List[Dict[str, str]]:
        ...
        if any(w in desc_lower for w in ['perspective', 'different', 'community', 'neighborhood', 'environment', 'culture']):
            connections.append({
        ...
        if any(w in desc_lower for w in ['learn', 'grow', 'change', 'reinvent', 'self-taught', 'pivot', 'career']):
            connections.append({
                'anchor': 'reinvention',
        ...
        if any(w in desc_lower for w in ['relationship', 'client', 'trust', 'authentic', 'long-term', 'compound']):
            connections.append({
                'anchor': 'ted_proof',
                'story': self.STORY_ANCHORS['ted_proof'],
                'connection': 'The Ted Foxman story proves this — authenticity over years compounds into real business',
            })

        # Standing out / doing your own thing → suburban_outsider
        if any(w in desc_lower for w in ['different', 'stand out', 'unique', 'own path', 'unconventional', 'versatile']):
            connections.append({
        ...
        if any(w in desc_lower for w in ['inequality', 'gap', 'underserved', 'funding', 'zip code', 'privilege']):
            connections.append({
        ...
        if any(w in desc_lower for w in ['faith', 'god', 'blessed', 'grateful', 'humble', 'prayer']):
            connections.append({
                'anchor': 'faith',
    ...
    def _find_strongest_connection(self, desc_lower: str, themes: List[str]) -> str:
        ...
        if 'business' in themes:
            return "CEO who learned business by doing — shooting videos in people's hometowns, building relationships that compound over years (Ted Foxman proof)."
    ...
    def _assess_audience_value(self, desc_lower: str, themes: List[str]) -> str:
        ...
        if 'technology' in themes:
            return "Makes AI/tech feel doable. Your aunt could use this. Your barber could use this."
        if 'business' in themes:
...
def main():
    ...
    parser.add_argument('--action', choices=['curate', 'angle', 'formats'],
                       default='curate', help='Action to perform')
