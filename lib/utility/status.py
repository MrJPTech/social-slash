...
def show_api_status():
    ...
    if google_key:
        print(f"    Gemini API:   CONFIGURED (key present)")
    else:
    ...
    if anthropic_key:
        print(f"    Anthropic:    CONFIGURED (key present)")
    else:
...
def main():
    ...
    parser.add_argument('--section', choices=['all', 'accounts', 'bots', 'api'],
                       default='all', help='Status section to display')
