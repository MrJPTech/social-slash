...
class TestCommentReplyGeneration:
    ...
    def test_generate_comment_reply(self, generator):
        ...
        reply = generator.generate_comment_reply(
            comment="Great post!", author="user123", platform="instagram"
        )
    ...
    def test_generate_reply_respects_max_length(self, generator):
        ...
        generator._ai_client.generate_content.return_value = MagicMock(text="A" * 5000)
...
class TestDMReplyGeneration:
    ...
    def test_generate_dm_reply(self, generator):
        ...
        reply = generator.generate_dm_reply(
            message="Hello!", sender_name="John", platform="instagram"
        )
        assert reply is not None
        assert isinstance(reply, str)
...
class TestEdgeCases:
    ...
    def test_empty_comment(self):
        ...
        gen = ResponseGenerator()
        gen._ai_client = MagicMock()
        gen._ai_client.generate_content.return_value = MagicMock(text="Thanks for engaging!")

        reply = gen.generate_comment_reply(comment="", author="user", platform="instagram")
    ...
    def test_very_long_comment(self):
        ...
        reply = gen.generate_comment_reply(
            comment=long_comment, author="verbose_user", platform="reddit"
        )
        assert reply is not None
    ...
    def test_special_characters_in_comment(self):
        ...
        gen = ResponseGenerator()
        gen._ai_client = MagicMock()
        gen._ai_client.generate_content.return_value = MagicMock(text="Thanks!")

        reply = gen.generate_comment_reply(
            comment="Love this! <3 @user #hashtag $$$", author="emoji_fan", platform="instagram"
        )
        assert reply is not None
